...
class FhevmDecryptionSignatureStorageKey {
  ...
  constructor(instance: FhevmInstance, contractAddresses: string[], userAddress: string, publicKey?: string) {
    ...
    const sortedContractAddresses = (contractAddresses as unknown as `0x${string}`[]).sort();
  ...
  get publicKey(): string | undefined { return this.#publicKey; }
...
export class FhevmDecryptionSignature {
  ...
  private constructor(parameters: FhevmDecryptionSignatureType) {
    if (!FhevmDecryptionSignature.checkIs(parameters)) { throw new TypeError("Invalid FhevmDecryptionSignatureType"); }
  ...
  static checkIs(s: unknown): s is FhevmDecryptionSignatureType {
    ...
    if (!("publicKey" in (s as any) && typeof (s as any).publicKey === "string")) return false;
    if (!("privateKey" in (s as any) && typeof (s as any).privateKey === "string")) return false;
    if (!("signature" in (s as any) && typeof (s as any).signature === "string")) return false;
    if (!("startTimestamp" in (s as any) && typeof (s as any).startTimestamp === "number")) return false;
    if (!("durationDays" in (s as any) && typeof (s as any).durationDays === "number")) return false;
    if (!("contractAddresses" in (s as any) && Array.isArray((s as any).contractAddresses))) return false;
    for (let i = 0; i < (s as any).contractAddresses.length; ++i) { if (typeof (s as any).contractAddresses[i] !== "string") return false; if (!((s as any).contractAddresses[i] as string).startsWith("0x")) return false; }
    if (!("userAddress" in (s as any) && typeof (s as any).userAddress === "string" && ((s as any).userAddress as string).startsWith("0x"))) return false;
    if (!("eip712" in (s as any) && typeof (s as any).eip712 === "object" && (s as any).eip712 !== null)) return false; if (!("domain" in (s as any).eip712 && typeof (s as any).eip712.domain === "object")) return false; if (!("primaryType" in (s as any).eip712 && typeof (s as any).eip712.primaryType === "string")) return false; if (!("message" in (s as any).eip712)) return false; if (!("types" in (s as any).eip712 && typeof (s as any).eip712.types === "object" && (s as any).eip712.types !== null)) return false; return true;
  }
  ...
  async saveToGenericStringStorage(storage: GenericStringStorage, instance: FhevmInstance, withPublicKey: boolean) {
    ...
    } catch { console.error(`FhevmDecryptionSignature.saveToGenericStringStorage() failed!`); }
  }
  ...
  static async loadFromGenericStringStorage(storage: GenericStringStorage, instance: FhevmInstance, contractAddresses: string[], userAddress: string, publicKey?: string): Promise<FhevmDecryptionSignature | null> {
    try {
      ...
      if (!result) { console.warn(`Could not load signature! key=${storageKey.key}`); return null; }
      try { const kps = FhevmDecryptionSignature.fromJSON(result); if (!kps.isValid()) { return null; } return kps; } catch { return null; }
    } catch { console.error(`FhevmDecryptionSignature.loadFromGenericStringStorage() failed!`); return null; }
  }
  ...
  static async new(instance: FhevmInstance, contractAddresses: string[], publicKey: string, privateKey: string, signer: ethers.Signer): Promise<FhevmDecryptionSignature | null> {
    try {
      ...
      const eip712 = instance.createEIP712(publicKey, contractAddresses as unknown as `0x${string}`[], startTimestamp, durationDays);
  ...
  static async loadOrSign(instance: FhevmInstance, contractAddresses: string[], signer: ethers.Signer, storage: GenericStringStorage, keyPair?: { publicKey: string; privateKey: string }): Promise<FhevmDecryptionSignature | null> {
    const userAddress = (await signer.getAddress()) as `0x${string}`;
    const cached: FhevmDecryptionSignature | null = await FhevmDecryptionSignature.loadFromGenericStringStorage(storage, instance, contractAddresses, userAddress, keyPair?.publicKey);
