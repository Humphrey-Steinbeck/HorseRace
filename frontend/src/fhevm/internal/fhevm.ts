...
import type { FhevmInstanceConfig, FhevmInstance } from "../fhevmTypes";
import { isFhevmWindowType, RelayerSDKLoader } from "./RelayerSDKLoader";
import { publicKeyStorageGet, publicKeyStorageSet } from "./PublicKeyStorage";

export class FhevmReactError extends Error { code: string; constructor(code: string, message?: string, options?: { cause?: unknown }) { super(message); if (options?.cause) { (this as any).cause = options.cause; } this.code = code; this.name = "FhevmReactError"; } }
...
const fhevmInitSDK = async (options?: { trace?: (message?: unknown, ...optionalParams: unknown[]) => void }) => { if (!isFhevmWindowType(window, console.log)) { throw new Error("window.relayerSDK is not available"); } const result = await (window as any).relayerSDK.initSDK(options); (window as any).relayerSDK.__initialized__ = result; if (!result) { throw new Error("window.relayerSDK.initSDK failed."); } return true; };

function checkIsAddress(a: unknown): a is `0x${string}` { if (typeof a !== "string") return false; if (!isAddress(a)) return false; return true; }
...
async function getChainId(providerOrUrl: Eip1193Provider | string): Promise<number> { if (typeof providerOrUrl === "string") { const provider = new JsonRpcProvider(providerOrUrl); return Number((await provider.getNetwork()).chainId); } const chainId = await providerOrUrl.request({ method: "eth_chainId" }); return Number.parseInt(chainId as string, 16); }
...
async function resolve(providerOrUrl: Eip1193Provider | string, mockChains?: Record<number, string>): Promise<ResolveResult> {
  ...
  const hasKey = Object.prototype.hasOwnProperty.call(_mockChains, chainId);
  if (hasKey) { if (!rpcUrl) { rpcUrl = _mockChains[chainId]; } return { isMock: true, chainId, rpcUrl }; }
...
export const createFhevmInstance = async (parameters: { provider: Eip1193Provider | string; mockChains?: Record<number, string>; signal: AbortSignal; onStatusChange?: (status: FhevmRelayerStatusType) => void; }): Promise<FhevmInstance> => {
  ...
  if (isMock) {
    const fhevmRelayerMetadata = await (async () => {
      try { const version = await getWeb3Client(rpcUrl!); if (typeof version === "string" && version.toLowerCase().includes("hardhat")) { return await getFHEVMRelayerMetadata(rpcUrl!); } } catch {}
      return undefined;
    })();
  ...
  const aclAddress = relayerSDK.SepoliaConfig.aclContractAddress; if (!checkIsAddress(aclAddress)) { throw new Error(`Invalid address: ${aclAddress}`); }
  const pub = await publicKeyStorageGet(aclAddress); throwIfAborted();
  const config: FhevmInstanceConfig = { ...relayerSDK.SepoliaConfig, network: providerOrUrl, publicKey: pub.publicKey, publicParams: pub.publicParams } as unknown as FhevmInstanceConfig;
  notify("creating");
