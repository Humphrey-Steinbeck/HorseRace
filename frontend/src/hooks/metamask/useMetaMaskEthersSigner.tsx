...
function useMetaMaskEthersSignerInternal(parameters: { initialMockChains?: Readonly<Record<number, string>> }): UseMetaMaskEthersSignerState {
  ...
  const sameChain = useRef((cid: number | undefined) => { return cid === chainIdRef.current; });
  const sameSigner = useRef((s: ethers.JsonRpcSigner | undefined) => { return s === ethersSignerRef.current; });
...
export const MetaMaskEthersSignerProvider: React.FC<MetaMaskEthersSignerProviderProps> = ({ children, initialMockChains }) => {
  ...
  return (<MetaMaskEthersSignerContext.Provider value={props}>{children}</MetaMaskEthersSignerContext.Provider>);
};
